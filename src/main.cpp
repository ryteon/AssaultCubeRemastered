...
void writepngchunk(stream *f, const char *type, const void *data = NULL, uint len = 0)
{
    ...
    // Prepare the buffers for CRC calculation
    ENetBuffer buffers[2];
    buffers[0].data = (void *)type;
    buffers[0].dataLength = 4;

    // If data exists, add it to the buffer
    if (data) {
        buffers[1].data = (void *)data;
        buffers[1].dataLength = len;
    }

    // Compute CRC32 using enet_crc32
    uint crc = crc32(0, Z_NULL, 0); // Initial CRC
    crc = ~enet_crc32(buffers, data ? 2 : 1);  // Use two buffers if data is provided

    // Write the inverted CRC value
    f->putbig<uint>(~crc);
}
